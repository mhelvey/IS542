...
# Conon's REGEX from his key
# RELEASE_NO = re.compile('Release No [^\s]*\s*(\d+)', re.IGNORECASE | re.DOTALL | re.MULTILINE)
# CASE_NAME = re.compile('Securities and Exchane Commission v\.\s*([A-Za-z0-9\s\.\,]+)', re.IGNORECASE | re.DOTALL)
# AMOUNTS = re.compile('(\$[0-9\s]+\s*(billion|million|thousand)\*)', re.DOTALL | re.MULTILINE)
